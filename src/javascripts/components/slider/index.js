...
export class SiemaWithDots extends Siema {
  ...
  updateDots() {
    ...
    for (let i = 0; i < this.dots.querySelectorAll("div").length; i++) {
      ...
      this.dots
        ...
        [i].classList[addOrRemove]("catalog-item__pagination-item--active");
    }
  }
  ...
  updatePreviews(container) {
    ...
    for (
      let i = 0;
      i < container.querySelectorAll(".catalog-item__preview-item").length;
      i++
    ) {
      // if current dot matches currentSlide prop, add a class to it, remove otherwise
      const addOrRemove = this.currentSlide === i ? "add" : "remove";
      container
        .querySelectorAll(".catalog-item__preview-item")
        [i].classList[addOrRemove]("catalog-item__preview-item--active");
    }
  }
}
